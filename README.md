...
5. Jetzt, falls du in deinem VS Code Explorer nach dem erfolgreichen Öffnen des Projektes einen .vscode findest, lösche ihn und gehe erneut in der Seitenleiste auf den ESP-IDF Reiter, klappe nun etwas weiter unten den "Advanced" Reiter aus und klicke auf den Eintrag "Add .vscode subdirectory files".
6. Ganz unten in der Leiste sollte "ESP-IDF v5.x.x" stehen. Daneben sollte ein Stern und "UART" stehen. Sollte das nicht der Fall sein (bspw. JTAG), klicke einmal auf den Text und wähle oben UART aus. Daneben sollte der richtige COM-Port ausgewählt werden. Klicke einmal auf COM und wähle den richtigen Port aus (es sollte nur einer in der Liste sein - mindestens einer mit ESP im Namen). Neben dem COM Port sollte esp32s3 ausgwählt sein. Steht dort nur esp32, klicke einmal drauf, wähle aus der Liste esp32s3 aus. Die Auswahl des Chips kannst du einfach ignorieren. Mit ESC kannst du die Auswahl schließen. 
7. Jetzt kannst du unten in der Leiste von Visual Studio auf das Schraubenschlüssel-Icon zum "Builden" des Projektes mit der ESP-IDF klicken und abwarten.
...
(Zusätzlicher Tipp: Falls es aus unerklärlichen Gründen mal nicht builden und laufen sollte, lösche den "build" Ordner und versuche es erneut zu bauen. Hat uns öfter geholfen :).)
